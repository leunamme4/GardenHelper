...
class RegisterFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.registerButton.setOnClickListener {
            if (binding.emailFieldRegister.text.toString().isEmpty() || binding.passwordFieldRegister.text.toString().isEmpty()) {
                ...
                FirebaseAuth.getInstance().createUserWithEmailAndPassword(
                    ...
                    if (task.isSuccessful) {
                        Toast.makeText(requireContext(), "Аккаунт зарегистрирован успешно", Toast.LENGTH_SHORT).show()
                        findNavController().popBackStack()
                    } else {
                        Toast.makeText(requireContext(), "Произошла ошибка, попробуйте снова", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
