package com.example.gardenhelper.ui.notifications.edit_notification
...
import com.example.gardenhelper.databinding.FragmentEditNotificationBinding
...
class EditNotificationFragment : Fragment() {
    ...
    private val viewModel by viewModel<EditNotificationViewModel>()

    private var _binding: FragmentEditNotificationBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEditNotificationBinding.inflate(inflater, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getNotification(requireArguments().getInt(NOTIFICATION_ID))
    }
    ...
    private fun render(notification: Notification) {
        binding.etNameText.setText(notification.title)
        binding.etDescriptionText.setText(notification.message)

        val time = notification.time.split(" ") // [0] - date, [1] - time
        binding.etDate.setText(time[0])
        binding.etTime.setText(time[1])
        binding.activeSwitch.isChecked = notification.isActive
    }
